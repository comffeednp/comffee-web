...
// Mock the webhook's dependencies so we can exercise the entity-routing fork:
// a payment that matches a reservation's initial intent must confirm the
// booking; one that matches a *balance* intent must settle the balance instead.
const h = vi.hoisted(() => {
  ...
  const tableResults: Record<string, unknown> = {
    branches: { data: { slug: "test-branch", name: "Test Branch", branch_rates: [] } },
  };
  return {
    confirmReservation: vi.fn(async () => {}),
...
vi.mock("@/lib/reservations", () => ({
  confirmReservation: h.confirmReservation,
...
vi.mock("@/lib/branch-instructions", () => ({ listInstructionPhotos: vi.fn(async () => []) }));
...
describe("PayMongo webhook — entity routing", () => {
  ...
  it("initial reservation payment → confirms the booking (not a balance settle)", async () => {
    h.getReservationByIntent.mockResolvedValue({
      id: "res_1", branch_id: "br_1", guest_email: null,
      check_in: "2026-07-01", check_out: "2026-07-03", num_guests: 2,
      total_php: 4650, guest_name: "Guest",
    });
    const res = await POST(paidEvent());
    const json = await res.json();
    expect(json.kind).toBe("reservation");
    expect(h.confirmReservation).toHaveBeenCalledWith("res_1");
  ...
  it("failed payment on a reservation → cancels the hold", async () => {
    h.getReservationByIntent.mockResolvedValue({
      id: "res_3", branch_id: "br_1", guest_email: null,
      check_in: "2026-07-01", check_out: "2026-07-03", num_guests: 2,
      total_php: 4650, guest_name: "Guest",
    });
