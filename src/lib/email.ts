...
function chrome({ preheader, bodyHtml, ctaLabel, ctaHref }: ChromeOptions): string {
  const logoBase = (process.env.NEXT_PUBLIC_SITE_URL || "https://www.comffee.org").replace(/\/$/, "");
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
        ...
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border-radius:16px;border:1px solid #e8dcc4;box-shadow:0 8px 24px rgba(26,15,6,0.06);overflow:hidden;">
          <tr>
            <td style="padding:24px 32px 0;">
              <img src="${logoBase}/comffee-logo-trimmed.png" alt="Comffee" width="44" height="56" style="display:block;border:0;outline:none;text-decoration:none;">
              <div style="margin-top:10px;font-family:'JetBrains Mono','SFMono-Regular',Menlo,Consolas,monospace;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#8a7a68;">
